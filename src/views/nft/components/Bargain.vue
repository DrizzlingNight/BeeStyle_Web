        ...
        <v-row class="d-flex justify-center">
          <v-col cols="11">
            <v-card-text class="white--text text-center" style="font-size: 24px">
              {{ $t("productDetail.bargainDialog.title") }}
            </v-card-text>
                ...
                <v-col class="pa-0">
                  <v-form @submit.prevent v-model="offerForm">
                    <VuetifyNumberInput
                        ...
                        :rules="offerRule"
                        :placeholder="$t('productDetail.biddingDialog.youPlaceholdor')"
                        backgroundColor=""
                    >
                    </VuetifyNumberInput>
            ...
            <v-col class="d-flex justify-space-between pa-0 mt-4">
              <v-col class="ETHColor">
                ...
                <div class="py-1">{{ $t("productDetail.bargainDialog.offerFee") }}</div>
                <div>{{ $t("productDetail.bargainDialog.freezePrice") }}</div>
              </v-col>
              <v-col class="text-right">
                <div v-if="!offerLowerPrice">{{ showBalance }} {{ NFTData.coin && NFTData.coin.name }}</div>
            ...
            <v-col cols="12" :class="offerLowerPrice ? 'mt-4':'mt-10'">
              ...
              <v-btn
                  ...
                  block
                  class="btnColor"
                  :disabled="!offerForm || offerPrice <= 0 || offerLowerPrice"
...
export default {
  ...
  computed: {
    ...
    //現金價值
    cash() {
      return this.rate ? numFormat(formatDecimal(Number(this.rate.rate) * Number(this.offerPrice), 4)) : '--'
    },
    //餘額
    totalPrice() {
      if (this.NFTData.coin) {
    ...
    //出價手續費
    offerInterest() {
      return formatDecimal((this.NFTsellFee * Number(this.offerPrice)), 4)
    },
    showFee() {
  ...
  methods: {
    ...
    showPasswordPage() {
      this.$emit("showPasswordPage",
          ...
          offerNFT,
          {price: this.offerPrice, token: this.$route.params.id, coin: this.NFTData.coin.id}
      )
    },
    goRecharge() {
