  ...
  <v-list two-line height="430px" class="overflow-y-auto list d-flex align-top" @scroll.native="historyScroll">
    ...
    >
      <template v-for="(item,index) in items">
        <v-list-item :key="item.title" @click="goMakeDeal(index)" :disabled="!isMine || isAuction">
          <template v-slot:default="{ active }">
            ...
            <v-list-item-content>
              <v-row class="d-flex justify-space-around">
                ...
                <show-price class="mr-8" :coin="coin" :nft-price="item.price" symbol="ETH"></show-price>
              </v-row>
            </v-list-item-content>

            <v-list-item-action v-if="isMine && !isAuction">
              <v-icon
                  v-if="!active"
...
export default {
  ...
  methods: {
    ...
    getNftBidList() {
      ...
      getNFTBidList({token: this.$route.params.id, offset: this.limit * this.page})
          .then(res => {
